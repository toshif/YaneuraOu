...
void random_player2(Position& pos, int loop_max)
{
  pos.init();
...
void user_test2(Position &pos, std::istringstream &is)
{
  int loop_max = 100000; 
  is >> loop_max;
  cout << "Random Player test , loop_max = " << loop_max << endl;
  random_player2(pos, loop_max);
  cout << "finished." << endl;
}
...
struct MovePicker
{
	...
	MovePicker(Position &pos, bool or_node)
	{
		...
		endMoves = or_node ? generateMoves<CHECKS_ALL>(pos, moves) : generateMoves<EVASIONS_ALL>(pos, moves);
...
void dfpn(Position &r)
{
	...
	sync_cout << "info string"
						<< " pn " << entry.pn << " dn " << entry.dn << " nodes_searched " << nodes_searched << sync_endl;
